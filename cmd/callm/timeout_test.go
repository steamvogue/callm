...
func TestTimeoutCLICommands(t *testing.T) {
	for _, command := range []struct {
		...
		{"orca", []string{"chat", "--no-stream", "--orca", "prompt"}},
		{"orca stream", []string{"chat", "--stream", "--orca", "prompt"}},
		{"orca models", []string{"models", "--orca"}},
		{"orca info", []string{"info", "--orca", "test-model"}},
		{"orca raw", []string{"raw", "--orca", "/endpoint", "{}"}},
