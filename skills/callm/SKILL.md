...
It connects to OpenAI-compatible gateways and the native Anthropic API, with millisecond startup and real-time streaming. These instructions describe v0.3.0+; check `callm --version` and `callm --help` when using an older installation.
...
Straitly/OpenRouter and `claude-sonnet-4-6` on Anthropic. Without an explicit
